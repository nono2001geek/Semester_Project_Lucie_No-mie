...
create_output_folders <- function() {
  ...
  dir.create("outputs/isochrones", recursive = TRUE, showWarnings = FALSE)
  dir.create("outputs/pois", recursive = TRUE, showWarnings = FALSE)
}
...
## Prepare isochrone and POI data for mapping

```{r prepare-isochrone-poi-data}
create_output_folders()

modes <- c("Walking", "Cycling")

isochrones <- readRDS("outputs/isochrones_15min.rds") |>
  st_make_valid() |>
  filter(mode %in% modes) |>
  st_transform(4326)
...
pois_unique <- pois_by_iso |>
  deduplicate_pois()

home_sf <- home_sf |>
  st_transform(4326)

pois_unique$poi_activity_category <- as.factor(pois_unique$poi_activity_category)

saveRDS(isochrones, "outputs/isochrones/isochrones.rds")
saveRDS(pois_by_iso, "outputs/pois/pois_by_iso.rds")
saveRDS(pois_unique, "outputs/pois/pois_unique.rds")
...
```{r static-map}
isochrones_2056 <- isochrones |>
  st_transform(2056)

pois_2056 <- pois_unique |>
  st_transform(2056)

home_2056 <- home_sf |>
  st_transform(2056)

map_pois_black <- ggplot() +
  geom_sf(
    data = isochrones_2056,
    fill = NA,
    color = NA
  ) +
  geom_sf(
    data = isochrones_2056 |> filter(mode == "Cycling"),
  ...
  geom_sf(
    data = isochrones_2056 |> filter(mode == "Walking"),
...
ggsave(
  filename = "outputs/plots/map_pois.png",
  plot = map_pois_black,
  width = 12,
  height = 6,
  dpi = 300
