...
```{r}
# Actual mobility: share of observed visits within 15 min
...
plot_data <- comparison_table |>
  ...
  pivot_longer(
    cols = c(share_walk_15, share_bike_15),
    names_to = "mode",
    values_to = "share_visits"
  ) |>
  mutate(
    mode = recode(
      mode,
      "share_walk_15" = "Walking",
      "share_bike_15" = "Cycling"
    ),
    n_pois = if_else(mode == "Walking", pois_walk_15, pois_bike_15),
    Activity = fct_reorder(Activity, share_visits, max)
  )

option_1 <- ggplot(plot_data,
       aes(x = share_visits, y = Activity, fill = mode)) +
  geom_col(
    position = position_dodge(width = 0.75),
    width = 0.65
  ) +
  geom_text(
    aes(label = paste0(round(share_visits, 1), "%")),
    position = position_dodge(width = 0.75),
    hjust = -0.15,
    size = 3.3
  ) +
  geom_text(
    aes(label = paste0("POIs: ", n_pois)),
    position = position_dodge(width = 0.75),
    hjust = 1.1,
    color = "black",
    size = 3
  ...
  labs(
    x = "Observed visits within 15 minutes",
    y = NULL,
    fill = "Transport mode"
  ) +
...
plot_ready <- plot_data |>
  mutate(
    n_poi = tidyr::replace_na(n_pois, 0),
    share_visits = tidyr::replace_na(share_visits, 0)
  ) |>
  select(mode, Activity, n_poi, share_visits) |>
  pivot_longer(
    cols = c(n_poi, share_visits),
    names_to = "metric",
    values_to = "value"
  ) |>
  mutate(
    metric = case_when(
      metric == "n_poi" ~ "Accessible POIs\n(absolute number)",
      metric == "share_visits" ~ "Actual visits inside\nisochrone (%)"
    ),
    label = case_when(
      metric == "Accessible POIs\n(absolute number)" ~ as.character(round(value, 0)),
      TRUE ~ paste0(round(value, 1), "%")
    ),
...
option_2 <-ggplot(plot_ready, aes(x = value, y = Activity, fill = mode)) +
  ...
  scale_fill_manual(values = c("Walking" = "#E69F00", "Cycling" = "#56B4E9")) +
  scale_x_continuous(
...
ggsave(
  filename = "outputs/plots/option_1.png",
  plot = option_1,
  width = 12,
  height = 6,
  dpi = 300
