...
# Anonymised map: isochrones + POI + real visits/trajectories
# Only visits within Zurich are kept


# 1. Make sure all objects are valid sf objects and use a metric CRS
visites_2056 <- visits |>
  st_transform(2056)

iso_2056 <- isochrones_15min |>
  st_transform(2056) |>
  st_make_valid()

pois_2056 <- pois_by_iso |>
  st_transform(2056) |>
  st_make_valid()
...
# 2. Create an approximate Zurich bbox
# This is roughly Zurich city and close surroundings
zurich_bbox <- st_as_sfc(
  st_bbox(
    c(
      xmin = 8.43,
      ymin = 47.31,
...
# 3. Keep only real visits located within Zurich bbox
visites_zurich <- visites_2056[
  lengths(st_within(visites_2056, zurich_bbox)) > 0,
]

# 4. Optional: remove duplicated POI caused by overlapping isochrones
...
if (length(dedup_cols) > 0) {
  pois_unique <- pois_2056 |>
    distinct(across(all_of(dedup_cols)), .keep_all = TRUE)
} else {
  pois_unique <- pois_2056
}

# 5. Create approximate trajectory lines by day
# This connects the visited points in chronological order.
# It is useful for visualisation, but does not represent the exact street path.
make_daily_line <- function(x) {
...
trajectory_lines <- visites_zurich |>
  filter(!is.na(start_time), !is.na(date)) |>
  arrange(date, start_time) |>
  group_split(date) |>
  map_dfr(make_daily_line)

# 6. Anonymise the map by centering everything around home
home_xy <- st_coordinates(home_2056)[1, ]

shift_to_home_origin <- function(x, home_xy) {
  x_shifted <- x
  st_geometry(x_shifted) <- st_geometry(x_shifted) - home_xy
  return(x_shifted)
}

visites_anon <- shift_to_home_origin(visites_zurich, home_xy)
iso_anon <- shift_to_home_origin(iso_2056, home_xy)
pois_anon <- shift_to_home_origin(pois_unique, home_xy)
home_anon <- shift_to_home_origin(home_2056, home_xy)

if (nrow(trajectory_lines) > 0) {
  trajectory_anon <- shift_to_home_origin(trajectory_lines, home_xy)
} else {
  trajectory_anon <- trajectory_lines
}

# 7. Plot anonymised map
ggplot() +
  ...
  # Approximate real trajectories
  geom_sf(
    data = trajectory_anon,
    color = "grey25",
    linewidth = 0.5,
    alpha = 0.45
  ) +
  ...
  geom_sf(
    data = pois_anon,
    aes(color = poi_category),
    size = 1.2,
    alpha = 0.65
  ) +
  
  # Real visited places within Zurich
  geom_sf(
    data = visites_anon,
    color = "black",
    size = 1.1,
  ...
  labs(
    title = "Anonymised map of 15-minute accessibility and observed mobility",
    subtitle = "Only visits within Zurich are shown. Home is centred at (0,0), without basemap.",
    fill = "Isochrone",
    color = "POI category"
  ) +
