...
library(leaflet)
library(mapview)
library(nnet) #to train a multinomial classification model. This means the model can predict several possible categories, for example
library(yardstick) #for evaluating model performance, but for your project we can also use simple tables.
```
...
```{r}
# creating a clean dataset just for the model 
...
# Predict activity categories for remaining (test data)
...
# compare manual vs predicted classification
...
# check correct vs wrong predictions
...
# use the results from previsou accuracy test.
```
...
n_correct <- sum(
  test_predictions$activity_category ==
...
prop.table(table(model_data$activity_category))
...
To evaluate the predictive performance of the activity classification model, a hold-out test dataset containing 81 observations was used. The model correctly classified 47 observations, resulting in an overall accuracy of 58.0%.

As a baseline, the majority-class classifier was considered. Since the activity category "home" represented 40% of all observations, a naive classifier assigning every observation to this category would achieve an accuracy of 40%.

An exact binomial test was conducted to assess whether the observed accuracy differed from the majority-class baseline. The test indicated that the model accuracy was significantly higher than the baseline accuracy (p = 0.0013). The estimated accuracy was 58.0%, with a 95% confidence interval ranging from 46.5% to 68.9%.

These results suggest that the model captures meaningful spatial and temporal patterns associated with different activity types and performs substantially better than a naive classification strategy.
