...
Due to the large amount of data required to generate the isochrones and load the POIs, the preprocessing was divided into two separate files. The code used to create the isochrones and extract the POIs has been submitted separately to the graders by email, as it was too large to commit to the GitHub repository. The remaining preprocessing steps are documented here:
https://nono2001geek.github.io/Semester_Project_Lucie_No-mie/Data_prepro_clean.html
...
knitr::include_graphics("outputs/plots/map_pois.png")
```

@fig-option-2 compares the number of points of interest (POIs) accessible within 15 minutes from the home location with the proportion of actual visits that occurred within these isochrones. @fig-pois-black spatially illustrates the distribution of POIs relative to the walking and cycling catchment areas.
